...
export default {
  ...
  props: {
    ...
    width_chart: VueTypes.number.def(400),
    height_chart: VueTypes.number.def(300),
    margin: VueTypes.number.def(10),
    ...
    size_grafics_name: VueTypes.string.def("14px"),
 ...
 mounted: function(){
  ...
  //   var dataset = data.map(function(d) {return d.score;});
   ...
   var crimesdata2 = [{"name":"DUI-LIQUOR","score":9919},{"name":"CHILD-OTHER","score":2385},
    ...
    //get for axis x data
    var nameset  = dataset.map(function(d) {return d.name;});
    ...
    var yScale = d3.scaleLinear()
        .domain([
                d3.min(dataset, function(d) { return d.score - 10; }),
                d3.max(dataset, function(d) { return d.score + 10; })
        ]).range([chartHeight, 0]);
